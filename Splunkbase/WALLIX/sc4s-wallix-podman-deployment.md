# Deploying SC4S for WALLIX Bastion — Podman
...
This runbook uses **Podman**. SC4S is runtime-agnostic and its documentation
supports several; if you would rather run the other common one, use
[`sc4s-wallix-docker-deployment.md`](sc4s-wallix-docker-deployment.md)
— the SC4S configuration is identical, only the unit file differs.

```
  WALLIX Bastion                SC4S host                    Splunk indexers
  ───────────────               ─────────                    ───────────────
  System >                      podman container              HEC :8088
...
Podman is the packaged, default container runtime on RHEL, Rocky and Alma
8+, so on most regulated estates there is nothing extra to install or licence.
It also has no persistent daemon: systemd supervises the container process
directly, which is one fewer long-running privileged service to justify at
review.

### One thing to be clear about

The unit in this guide is a systemd **system** unit, so the container runs
as root.
Podman's rootless mode is a separate exercise and it collides with syslog:
an unprivileged process cannot bind a port below 1024. This guide puts WALLIX
on 2242, so the constraint does not bite — but it will the moment you also want
SC4S listening on 514.
...
comment out the `podman pull` line. That `ExecStartPre` runs on every start and
...
`podman pull`, edit the tag, `systemctl restart sc4s`.
...
does not exist until that finishes, so `podman logs` reports
`No such container: SC4S` in the meantime. Wait for it to appear:

```bash
sudo podman ps --filter name=SC4S
```
...
```bash
sudo podman exec SC4S env | grep WALLIX     # the variable reached the container
...
Three events - a privileged session, a failed authentication and a
configuration deletion - covering three of the seven families and three
of the four data models. Send them from any host that can reach the
listener:

```bash
TS=$(date '+%b %e %H:%M:%S')
cat <<EOF | nc -w2 sc4s.corp.example 2242
<14>$TS bastion-01 sshproxy[13656]: [SSH Session] session_id="1830cbf7a55a11dd005056b01296" client_ip="192.0.2.10" target_ip="srv-app-03.corp.example" user="jdoe" device="SRV-APP-03" service="SSH" account="root" type="SESSION_ESTABLISHED_SUCCESSFULLY"
<46>$TS bastion-01 WAB(root)[47165]: [wabauth] action="authentify" user="jdoe" client_ip="192.0.2.10" status="failure" infos="diagnostic ['local' -password- authentication failed]"
<46>$TS bastion-01 WAB(root)[47165]: [wabaudit] action="delete" type="User" object="tmpuser" user="admin" client_ip="192.0.2.20" infos="cn [tmpuser], profile [user]"
EOF
```

They are stamped with the current time, so they land in the last minute.
If they arrive and your appliance traffic does not, the problem is
between the Bastion and the listener, not in Splunk.
...
| Every `systemctl restart sc4s` takes about a minute | The `ExecStartPre=/usr/bin/podman pull` line contacts ghcr.io on each start. Comment it out and pin the image tag. |
| `podman logs SC4S` says `No such container: SC4S` | On a first start, the image pull is still running — check `systemctl status sc4s` and wait. If the unit is `failed`: `/opt/sc4s/env_file` missing, the `splunk-sc4s-var` volume not created, or the pull blocked from ghcr.io. |
...
| Nothing in `index=wallix_bastion` | SC4S cannot reach HEC. `podman logs SC4S` shows the TLS or connection error. Check the token, the port, and that HEC is on the **indexers**. |
| Batches rejected with HTTP 400 in `podman logs SC4S` | The token has a populated *Selected indexes* list and something SC4S sends is not on it — usually `main` or `_metrics`. HEC drops the entire batch, so good events disappear with the bad one. Blank the list. |
...
- [SC4S — Podman + systemd](https://splunk.github.io/splunk-connect-for-syslog/main/gettingstarted/podman-systemd-general/)
